...
with source as (
    ...
    -- 1. Incremental Filter: Only process new data
    {% if is_incremental() %}
...
-- 2. Deduplicate the INCOMING BATCH
-- If a device reports multiple times in this batch, keep the latest one.
-- dbt will then merge this latest version into the target table.
